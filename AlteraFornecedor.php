...
if (!empty($_POST)){
	...
	if (empty($_GET['id']) || empty($_POST['nome']) || empty($_POST['sexo']) || empty($_POST['endereco']) || 
		...
		if ($sexo === "Informe seu sexo"){
			...
			if (!$retEmail){
				?><script type="text/javascript">
						alert('CNPJ ou email não comferem, tente novamente!'); window.location="AlteraFornecedor.php?id="<?php echo $id; ?>
				</script> <?php
			}
...
<a href="ListaFornecedor.php" class="btn btn-info">Cancelar Alteração</a>
